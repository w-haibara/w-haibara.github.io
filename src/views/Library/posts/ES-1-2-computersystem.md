...
### 分散処理システムにおける障害透明性
* p54
* 利用者が，個々のコンピューターに障害が起きていることを認識することなく，システムを利用できること．

### ライブマイグレーション
* p55
* 仮想サーバで稼働しているOSやソフトウェアを停止することなく，他の物理サーバに移し替える技術．

### シェアードエブリシング
* p56
* アクティブ―アクティブ構成によって負荷分散を行うことによってサーバリソースの有効活用が可能となり，さらにデータが共有されているので，1台のサーバーの障害発生時でも処理を継続することができる．

### クライアントプログラムとサーバのデータ転送機構 (RPC)
* p57
* RPC (Remoe Procedure Call): ネットワークを介して接続された他のコンピュータのサブルーチンを実行することを可能にする技術 

### Webシステムの稼働監視
* p58
* ロードバランサが定期的に行うアプリケーションレベルの稼働監視
  * Webサーバの特定のURLにアクセスし，その結果に含まれる文字列が想定値と一致するかどうか確認する．
  
### RAID1~5の方式の違い
* p59
* データおよび冗長ビットの記録方式と記録位置との組み合わせ

### フェールオーバー処理
* p60
1. ネットワークが正常か確認
2. ネットワークに高負荷がかかっていないか確認
3. 本番系サーバの論理ドライブの占有権を奪い，ロックを掛ける
4. 本番系サーバが自らシャットダウン

### フェールセーフ
* p62
* システムに障害が発生したときでも，常に安全側にシステム
を制御する．
...
### NICのチーミング
* P64
* 一つのIPアドレスに複数のNICを割当て，負荷分散．帯域の有効活用，および耐障害性の向上を図る

### ターンアラウンドタイム
* p65
* ターンアラウンドタイム：処理要求を送ってから，処理が終了するまでの時間

### 性能改善手法的用後の性能比
* p66
* (性能改善手法的用後の性能比) = 1 / ( (1-R) + R/A )
  * R: 性能改善手法を適用した部分の全体に対する割合(0<R<1)
  * A: Rの性能改善手法的用後の性能比
  * これをアムダールの法則という
  
### コンピュータの性能評価(シミュレーションと解析的手法)
* p67
* シミュレーションを用いた手法では，解析的に解が求められないモデルに対しても，数値的に解が求まる．

### キャパシティプランニング
* p68
* 必要となるリソースを適切なタイミングで増強するによって，適正なコストで最適なパフォーマンスのサービスを提供する．

### 物理サーバのスケールアウト
* p69
* サーバのメモリを増設することによって，単位時間当たりの処理能力を向上させること．

### RASISのうち，Integrity (完全性) の例
* p70
* 排他制御を行うことによって，複数の利用者が同時にデータベースの更新処理を行う場合でも，データの整合性を保証する．

### 4種類の装置で構成されるシステムの稼働率
* p71
* (複数の装置で構成されるシステムの稼働率) = (少なくとも1台が稼働する確率)
* 稼働率 = 1 - 故障率
* (装置A・Bから成るシステムの稼働率) = 1 - (装置Aの故障率) x (装置Bの故障率)

### システムのアベイラビリティ(稼働率)
* p73
* (アベイラビリティ) = MTBF / ( MTBF + MTTR )
  * MTBF (Mean Time Between Failure): 平均故障間動作時間
  * MTTR (Mean Time To Repair): 平均修復時間
  
### MTTRを短くするもの
* p74
* エラーログ取得機能
  * 障害発生時に，エラーログを参照することで，修復時間(MTTR)を短くすることが期待できる．

## ソフトウェア
### タスクのプリエンプション
* p76
* タスクの状態遷移
  * 未登録状態・休止状態・実行可能状態・実行状態・待ち状態
* ディスパッチ
  * 実行状態・実行可能状態で遷移させること
* プリエンプション (タスクの横取り)
  * 実行状態のタスクの実行権を横取り(プリエンプト)して実行準備状態に移し，別の優先度の高いタスクを実行状態に移す．

### スレッドとプロセス
* p77
* 同一アドレス空間に含まれているスレッド間の通信は，プロセス間の通信と比較して効率的に行われる．
* コンテキスト
  * 実行中のタスクが持つ，レジスタ値などのCPUの状態．
* コンテキストスイッチ
  * 実行状態のタスクのコンテキストを保存し，次に実行するタスクのコンテキストを読み込むことで，タスクを切り替えること．
...
### タスクの優先度逆転
* p81
  * 優先度の低いタスクAと優先度の高いタスクBがあるとする．
  * Aが実行状態で，ある資源を利用しているときに，Bが実行可能状態となり，
  CPUの使用権がBに移っても，排他制御によってAに割り当てられた資源を奪うことはできないため，
  Bはすぐに待ち状態になってしまう．このとき，より優先度の高いBが，Aの終了を待つことになるため，優先度逆転が起きる．

### セマフォ
* p82
* 排他制御が必要な資源管理に使う同期制御方式．
  * P操作
  * V操作
